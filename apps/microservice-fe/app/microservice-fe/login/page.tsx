...
import { login, setStoredToken, StorefrontError } from "@/app/microservice-fe/lib/client";
...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f6f1eb] pb-12">
      ...
      >
        <div className="grid w-full gap-8 lg:grid-cols-[0.9fr,1.1fr]">
          <div className="rounded-[36px] border border-white/60 bg-[rgba(255,255,255,0.58)] p-8 shadow-md backdrop-blur-md">
            <p className="text-xs uppercase tracking-[0.28em] text-[#a1876b]">Welcome back</p>
            <h1 className="mt-5 text-5xl font-semibold tracking-tight text-[#1e1e1e]">Sign in to continue shopping.</h1>
            <p className="mt-5 max-w-md text-base leading-7 text-[#6b6b6b]">
              Your token unlocks the protected product and cart flows powered by the auth and product services.
            </p>
          </div>
          ...
          >
            ...
            <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <AnimatedButton type="submit" loading={loading} className="min-w-40">
                Sign in
              </AnimatedButton>
              <Link href="/microservice-fe/signup" className="text-sm font-medium text-[#8a7258] underline-offset-4 hover:underline">
                Need an account?
              </Link>
            </div>
          </motion.form>
